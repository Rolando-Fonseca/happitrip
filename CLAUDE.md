...
Read `docs/kit/system-prompt.md` for your personality and communication rules. Follow them throughout.

## Language

Detect the user's language from their first message. If they write in Spanish, conduct the ENTIRE flow in Spanish:
- Read `docs/kit/questionnaire-es.md` instead of `docs/kit/questionnaire.md`
- Read `docs/kit/system-prompt-es.md` instead of `docs/kit/system-prompt.md`
...
|--------|-----------|------------|
| **Motion** — easing, duration, springs, stagger, gestures | `animate` → `emil-design-eng` → `apple-design` | `frontend-design/reference/motion-design.md` and `docs/kit/design-guide.md` § Motion |
| **Typography, color, layout, the AI Slop Test** | `frontend-design` + `ui-ux-pro-max` + `docs/kit/design-guide.md` | `apple-design` § Typography has **no** say here — it recommends system fonts, which this repo bans |
...
See `docs/kit/skill-reference.md` for full invocation examples and all `--domain` values.
...
### Phase 1: Discovery
Read `docs/kit/questionnaire.md` (or `docs/kit/questionnaire-es.md` for Spanish). Ask questions conversationally in 4 rounds. Use smart defaults for anything the user skips or says "you decide."
...
Use `ui-ux-pro-max` to generate specific recommendations. If it fails, fall back to `docs/kit/design-guide.md` — pick colors from the industry palette table, fonts from the vibe pairing table, and tell the user what you chose and why.
...
- Page archetype from `docs/kit/landing-page-patterns.md` (explain why it fits their business)
...
#### Design & Code
- Apply `frontend-design` skill guidelines (or `docs/kit/design-guide.md`)
...
- See `docs/kit/performance-checklist.md` for Core Web Vitals optimization
- See `docs/kit/accessibility-checklist.md` for WCAG AA compliance
- Run ALL copy through `humanizer` skill (or manually check against AI patterns in `docs/kit/design-guide.md`)
- Use Google Fonts via `next/font/google` with `display: "swap"` and CSS variables

#### Section Order
Use the archetype from `docs/kit/landing-page-patterns.md` that best fits the user's business type. Tell the user which archetype you chose and why: "Based on your [business type], I'm using the [Archetype] pattern because [reason]." Default order: Hero > Features/Services > Social Proof > CTA > Footer.
...
See `docs/kit/deployment-guide.md` for troubleshooting.
...
See `docs/kit/design-guide.md` for the full reference. Critical rules:
